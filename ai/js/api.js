(function (g) {
  ...
  function friendly(err) {
    ...
    if (/Failed to fetch|NetworkError|load failed/i.test(m)) return '网络/CORS 失败：接口未允许跨域，可在设置里填写代理 URL。';
    ...
    if (err && err.status === 401) return '401：API Key 无效或已过期。';
    if (err && err.status === 402) return '402：余额不足。';
    if (err && err.status === 404) return '404：Base URL 或模型名不对，检查是否漏了 /v1。';
    if (err && err.status === 429) return '429：触发限流，稍后重试。';
  ...
  function stripThinking(text, reasoning) {
    ...
    if (s.length < 300 && !hasReason) return s.trim();
    if (hasReason) {
      ...
      for (var len = Math.min(r.length, head.length); len >= 24; len--) {
        if (r.slice(0, len) === head.slice(0, len)) { s = s.slice(offsetOf(s, len)).replace(/^\s+/, ''); break; }
      }
    }
    var lines = s.split('\n'), first = null;
    for (var k = 0; k < lines.length; k++) { if (lines[k].trim()) { first = lines[k]; break; } }
    var gate = hasReason || s.length >= 240 || (first && thinkLevel(first) === 2 && first.trim().length >= 60);
    if (gate) {
      var drop = 0, chars = 0;
  ...
  function isReasonerModel(name) { return /reasoner|thinking|-r1\b|r1-|deepseek-r/i.test(String(name || '')); }
  ...
  function readStream(res, onDelta, onThink, onEnd) {
    ...
    function fireEnd() { if (ended) return; ended = true; try { if (onEnd) onEnd(); } catch (e) {} }
      ...
      (function pump() {
        reader.read().then(function (r) {
          ...
          for (var i = 0; i < lines.length; i++) {
            ...
            var rc = d.reasoning_content != null ? d.reasoning_content : (d.reasoning != null ? d.reasoning : null);
  ...
  function chat(opts, onDelta, onThink) {
    ...
    var body = { model: opts.model || 'deepseek-chat', messages: msgs, stream: !!opts.stream, max_tokens: Number(opts.maxTokens) || 8000 };
    if (!reasoner) body.temperature = (opts.temperature === undefined ? 0.7 : Number(opts.temperature));

    var timeoutMs = Number(opts.timeoutMs) || 300000;
    var maxRetry = opts.retries === undefined ? 2 : Number(opts.retries);
    var attempt = 0, hide = opts.hideThinking !== false;

    function finalize(r) {
      var raw = r.text || '';
      var clean = hide ? stripThinking(raw, r.reasoning) : raw;
      return {
        text: clean, rawText: raw, reasoning: r.reasoning || '', usage: r.usage || null,
        finishReason: r.finishReason || null, truncated: r.finishReason === 'length',
        filtered: clean !== raw, empty: !String(clean).trim()
      };
    }

    function run() {
      attempt++;
      var ctl = new AbortController(), settled = false, stalled = false;
      ...
      return fetch(url, { method: 'POST', headers: headers, body: JSON.stringify(body), signal: ctl.signal })
        .then(function (res) {
          if (!res.ok) return res.text().then(function (t) { throw HttpError('HTTP ' + res.status + ' · ' + errText(t), res.status, t); });
          if (!body.stream || !res.body || !res.body.getReader) {
            return res.json().then(function (j) {
              var ch = (j.choices && j.choices[0]) || {}, msg = ch.message || {};
              var r = { text: msg.content || '', reasoning: msg.reasoning_content || msg.reasoning || '', usage: j.usage || null, finishReason: ch.finish_reason || null };
  ...
  g.AiApi = {
    chat: chat, endpoint: endpoint, proxied: proxied, friendly: friendly,
    estimateTokens: estimateTokens, stripThinking: stripThinking,
    isReasonerModel: isReasonerModel, mergeSystem: mergeSystem
  };
})(window);
