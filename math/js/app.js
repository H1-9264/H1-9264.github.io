...
(function() {
    ...
    function drawFunction(fn) {
        // 如果是隐函数，使用轮廓绘制
        if (isImplicitFunction(fn.expr)) {
            drawImplicitFunction(fn);
            return;
        }
    ...
    function drawImplicitFunction(fn) {
        const { width, height } = getCanvasSize();
        const offset = getCanvasOffset();
        const ctx = state.ctx;
        ...
        for (let py = Math.floor(offset.y); py < offset.y + height; py += resolution) {
            for (let px = Math.floor(offset.x); px < offset.x + width; px += resolution) {
                const x = canvasToX(px + resolution/2);
                const y = canvasToY(py + resolution/2);

                const val = evaluateImplicitFunction(fn.expr, x, y);
    ...
    // ==================== 函数解析 ====================
    function isImplicitFunction(expr) {
        return expr.includes('=');
    }

    function parseImplicitFunction(expr) {
        // 转换为 f(x,y) = 0 的形式
        const parts = expr.split('=');
        if (parts.length !== 2) return null;

        let left = parseFunction(parts[0]);
        let right = parseFunction(parts[1]);

        // 返回 f(x,y) - g(x,y)
        return `(${left}) - (${right})`;
    }

    function evaluateImplicitFunction(expr, x, y) {
        try {
            const parsed = parseImplicitFunction(expr);
            if (!parsed) return null;

            const result = new Function('x', 'y', `return ${parsed}`)(x, y);
    ...
    function downloadImage() {
        ...
        // 计算实际像素的偏移和尺寸
        const srcX = offset.x * dpr;
        const srcY = offset.y * dpr;
